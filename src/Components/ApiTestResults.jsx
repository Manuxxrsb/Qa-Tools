                ...
                <table className="min-w-full divide-y divide-gray-600">
                    <thead className="bg-gray-700">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Resultado</th>
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Detalles</th>
                        </tr>
                    </thead>
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {results.map((result, index) => (
                            <tr key={index} className={result.error ? 'bg-red-900/20' : 'hover:bg-gray-700'}>
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-300">{result.codigoRecibido || '-'}</td>
                                <td className="px-4 py-3 whitespace-nowrap">
                                    {result.error ? (
                                        <span className="text-red-400">✗ Error</span>
                                    ) : (
                                        <span className={result.coincide ? 'text-green-400' : 'text-yellow-400'}>
                                            {result.coincide ? '✓ OK' : '⚠ Diferente'}
                                        </span>
                                    )}
                                </td>
